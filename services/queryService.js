...
function getTable(workbook, tableName) {
...
function getColumns(workbook, tableName) {
  const sheet = workbook.Sheets[tableName];
...
function query(userId, options) {
  ...
  let rows = getTable(workbook, table);
  ...
  if (Array.isArray(joins) && joins.length) {
    for (const join of joins) {
      ...
      const rightRows = getTable(workbook, join.table);
